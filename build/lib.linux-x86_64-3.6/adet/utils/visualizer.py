...
class TextVisualizer(Visualizer):
    def draw_instance_predictions(self, predictions, path = ''):
        ...
        # print(path)
        # a = '/content/drive/MyDrive/sample_input/im0001.jpg'
	file_name = './txt_output/' + path.split('/')[-1][:-4] + '.txt'
        ...
        with open(file_name, 'w') as f:          
          for bezier, rec, score in zip(beziers, recs, scores):
            ...
            # print("Point: ", polygon, "Text: ", text , "\n-----------")
            # f.write(str(polygon).rstrip() + ',' + str(text) + '\n')
            f.write(str(polygon).rstrip() + '\n')
